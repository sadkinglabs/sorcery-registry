...
- **sync** - lists, then downloads what changed (a new file, a new MD5, a new recipe), renders the renditions, uploads them to the bucket's `images/` prefix, verifies every held object through the CDN, regenerates the export and opens a pull request. Nothing reaches `main` except through that PR. `limit` caps a run (the first full run is ~3.3 GB of originals; it resumes where it stopped because `data/images.json` is saved after every file).
