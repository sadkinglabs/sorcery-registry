...
| `origin` | cards, printings and sets | `api` when the official API serves the record, `manual` when the registry recorded it by hand (`data/manual.json`). Every record until now is `api` |
