...
```jsonc
{
  ...
  "sets":          [ { "set_code": "001", "set_name": "Alpha", "released_at": "2023-06-22",
                       "cards": 407, "printings": 817, "kind": "release", "origin": "api",
  ...
  "printings":     [ { "printing_id": "P000001", "codex_id": "C000001", "card_name": "Apprentice Wizard",
                       ...
                       "released_with": "001", "product": "Booster", "finish": "Standard", "slug": "001-apprentice_wizard-b-s",
...
- **Key on the IDs, treat everything else as data.** `slug`, `set_code`, `set_name`, `card_name` are conveniences that can change; `codex_id` and `printing_id` cannot. Sets are identified by their two official facts: `set_code` (001 = Alpha, 002 = Beta, 006 = Gothic; 003 is deliberately unused, so the codes are labels, not an order) and `set_name`, the official display name. Both are published exactly as upstream states them. The one exception is a set upstream will never serve, such as the curios: the registry gives it a code of its own, always three capital letters (`CUR`), so it can never be mistaken for one of the publisher's three-digit codes. A code is a label and never a number, so nothing is read from it: each set's `kind` says what it is. `release` is a set release, `promo` is the publisher's bucket for promos (999), and `registry` is a set of the registry's own. The kinds are recorded in [`data/sets.json`](data/sets.json), and a new set stops the release until someone classifies it.
...
- **Every record says where it lives.** Cards, printings and sets carry `api_url` (the record's own JSON object on `api.kairosarchive.net`) and `kairos_url` (its page on `kairosarchive.net`), derived from the id at export time so they can never name a different record - CI checks that too. `api_url` points at the moving major alias (`/v3/`), which always redirects to the newest verified v3.x release: follow it from any copy, however old, and you reach the current record. A consumer that wants the snapshot it fetched uses that release root's paths instead. Printings (and cards, through their default printing) also carry `image_urls` - the four renditions `small` (146×204), `normal` (488×680), `large` (672×936) as WebP, and `original`, the source file untouched (the publisher's, or for a printing recorded by hand, the photograph the archive was given) - and `image_status`: `missing` while the registry holds no image, `lowres` when the publisher's file (380×531 for the early sets) was upscaled to fill the large rendition, `ok` at full size. Image addresses are permanent: the name carries the printing id and an art-version key (`P000937.ab12cd34ef56.normal.webp`), and new art or a new encoding gets a new name rather than new bytes at the old one. What the registry holds is recorded in [`data/images.json`](data/images.json), with the file each image came from. The committed `registry.json` therefore contains `kairosarchive.net` addresses, and the GitHub mirror points at the domain by design.
- **Notes hold what the official API does not say.** Every card and printing carries `notes`, usually empty: facts the API has no place for, such as a promo that was prize support in a particular store kit. Each note gives its `source` and the date it was `recorded`. Notes are kept by hand in [`data/notes.json`](data/notes.json), and CI checks every note sits on a real record. A note never contradicts a field: a fact that fits a field is a correction, made through [`data/overrides.json`](data/overrides.json). Notes are reports, not official data, so quote the source with the fact.
- **Some records are kept by hand.** The official API does not serve everything that was printed: store-kit prize cards, Kickstarter pledge cards and curios exist only on tables and in binders. The registry records them by hand in [`data/manual.json`](data/manual.json), with the same fields as every other record and ids from the same counters. Each record says who stands behind it: `origin` is `api` when the API serves it and `manual` when the registry recorded it, and `manual` gives the source and the date. When the API later serves a manual record and a person confirms the match, the record keeps its id and becomes `api`, and `manual.confirmed_at` records the day. A manual record found to be wrong is never deleted; `manual.withdrawn` marks it. A manual printing's slug is the registry's prediction in the publisher's own shape and may change when the record is confirmed, so treat it like every slug: a lookup, never a key.
- **`released_with` names the set release a printing belongs to.** Upstream files every promo under set 999, but a store-kit promo is part of a set's release. For a printing in a set whose `kind` is `release`, `released_with` is that set. For a promo or a curio, it is the release recorded by hand in [`data/released-with.json`](data/released-with.json) or `data/manual.json`, and null until someone records it. So "everything that came out with Beta" is one filter.
